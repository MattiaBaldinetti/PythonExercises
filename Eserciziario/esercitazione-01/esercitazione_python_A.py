...
def check_int_str(stringa, intero):
    lista = list(stringa) #trasformo la stringa in una lista di stringe
    numero_stringa = str(abs(intero)) #trasformo l'intero in input in una stringa
    conta_occorrenze = lista.count(numero_stringa) #conto il numero che si ripete l'intero in input
...
def count_sub_string(query, corpo):
    ...
    for i in range(corpo_length - query_length + 1):
        if corpo[i:i+query_length] == query:
            count += 1
...
def count_sub_string_idx(query, corpo):
    ...
    query_length = len(query)
    corpo_length = len(corpo)
    print(corpo_length - query_length + 1)
    for i in range(corpo_length - query_length + 1):
        if corpo[i:i+query_length] == query:
            lista.append((i,i+query_length-1)) #aggiungo la tupla alla lista
...
def list_multipli(L):
    ...
    result = multiples_of_2 + multiples_of_5
    return result
...
def get_list_except_min_max_general(L):
    ...
    maxValue = max(L)
    minValue = min(L)
    output = [L.remove(x) for x in [c for c in L] if x == maxValue or x == minValue]
    lunghezza = len(L)
    return length - lunghezza
...
def reshape_by_index(L, src, dst):
    max_dst = max(dst, default=0)
    # valore max nella lista dst
    out = [None] * (max_dst + 1)
...
def list_to_tuple(L):
    ...
    for i in range(len(tupla)):
        for num in tupla[i]:
            molt *= num
        if molt%2 == 0:
            nuova_lista.append(list(tupla[i][::-1]))
        molt = 1
    return tuple(nuova_lista)
...
def sort_by_str(T):
    tupla = tuple(sorted(T, key=lambda s: (-len(s), s), reverse=True))
    return tupla
...
def sort_by_month(L):
    ...
    return sorted(L, key=lambda x: mesi.index(x.lower()))
...
# creo il dizionario dei conteggi degli elementi
def hist(values):
    conti = {}
    for v in values:
...
def int_to_hist(values):
    conti = hist(values)
    stringa = ''
    # potrebbero esserci dei buchi quindi scandisco i valori dal min al max+1
    for i in range(min(values), max(values)+1):
        n = conti.get(i, 0)
        stars = '*'*n
        stringa += f"{i}\t{stars}\n"   # e aggiungo la riga con le stelline
    return stringa
...
def anagramma(S, T):
    listaS = list(S)
    listaT = list(T)
    ordinataS = sorted(listaS)
    ordinataT = sorted(listaT)
    if (ordinataS == ordinataT):
        return True
    else:
        return False
